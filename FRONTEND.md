...
| `training_time` | string `"HH:MM:SS"` | Tiempo de entrenamiento |
| `check_time` | string `"HH:MM:SS"` | Tiempo de chequeo |
...
Send the 7 fields (snake_case) in the request payload (form-data). Counts (`takeoff_*`, `landing_*`) as numbers; times (`training_time`, `check_time`) as `"HH:MM:SS"` strings; `type` as one of the 5 allowed string values.
